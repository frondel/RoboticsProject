...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
   ...
   /*
     * INSERT CODE HERE
	*red detected at: hue value (0, 15)
    */
	...
	//Calculate moments of mask
        ...
        //Calcualte center of mass using moments
        center_of_mass.x = moments.m10 / moments.m00;
        center_of_mass.y = moments.m01 / moments.m00;

        //debug statement to check the coordinates
        ROS_INFO("%d %d", center_of_mass.x, center_of_mass.y);
	

	//Coner the image back to BGR
	cv::cvtColor(mask, dst, CV_GRAY2BGR);

	//function that draws red circle where the center of mass is
        cv::circle(dst, center_of_mass, 10, cv::Scalar(0,0,255), -1);
...
int main(int argc, char **argv)
{
  ...
  //this configuration detects red
  nh_.param<int>("hue_lower", hue_lower, 0);
  nh_.param<int>("hue_upper", hue_upper, 180);


  //this configuration detects green
  nh_.param<int>("sat_lower", sat_lower, 50);
  nh_.param<int>("sat_upper", sat_upper, 255);

  //this configuration detects blue
  nh_.param<int>("value_lower", value_lower, 100);
